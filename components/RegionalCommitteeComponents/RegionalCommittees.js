...
const RegionalCommittees = () => {
    ...
    const fetchPosts = async () => {
        try {
            // Replace 'your_acf_category_field' with the actual name of your ACF category field            const result = await fetch('https://ficac.ivistaz.co/wp-json/wp/v2/committiee?categories=8&per_page=100');
            const result = await fetch('https://ficac.ivistaz.co/wp-json/wp/v2/committiee?categories=8&per_page=100&_embed');

            // const result = await fetch('https://ficac.ivistaz.co/wp-json/wp/v2/committiee?categories=7?_embed');
            if (!result.ok) {
